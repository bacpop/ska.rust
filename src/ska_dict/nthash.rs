//! ntHash is a hash function tuned for a DNA alphabet {A, C, T, G}.
...
//! Also with thanks to @luizirber's ntHash [crate](https://crates.io/crates/nthash)
//!
...
impl NtHashIterator {
    ...
    /// Add a new base, remove a base from the end
    ...
    pub fn extra_hash(&self, extra_idx: usize) -> u64 {
        let mut new_hash = self.curr_hash() * ((extra_idx ^ self.k) as u64 * MULTISEED);
