...
impl TestSetup {
    pub fn setup() -> Self {
        ...
        wd.child(SYM_IN).symlink_to_dir(Path::new(FILE_IN).canonicalize().expect("Could not link expected files")).unwrap();
        wd.child(SYM_TEST).symlink_to_dir(Path::new(FILE_TEST).canonicalize().expect("Could not link expected files")).unwrap();
        Self{wd}
    }
    ...
    pub fn file_path(&self, name: &str, file_type: TestDir) -> PathBuf {
        match file_type {
            TestDir::Input => PathBuf::from(&format!("{}/{}/{}", self.wd.path().display(), SYM_IN, name)),
            TestDir::Correct => PathBuf::from(&format!("{}/{}/{}", self.wd.path().display(), SYM_TEST, name))
        }
    }

    pub fn file_string(&self, name: &str, file_type: TestDir) -> String {
        self.file_path(name, file_type).to_str().expect("Could not unpack file path").to_owned()
    }
